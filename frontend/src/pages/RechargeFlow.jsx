...
const RechargeFlow = () => {
  ...
  const handlePayment = async () => {
    setLoading(true);
    try {
      const email = localStorage.getItem('flashsim_email') || 'guest@flashsim.tn';
  ...
  const filteredOperators = operators.filter(op => op.region === selectedRegion);
              ...
              <div className="flex flex-row lg:flex-col justify-between lg:justify-start lg:space-y-6 relative">
                ...
                <div className="hidden lg:block absolute left-[19px] top-2 bottom-2 w-0.5 bg-slate-100 -z-10">
                  <div 
                    ...
                    style={{ height: step === 'region' ? '0%' : step === 'operator' ? '33%' : step === 'amount' ? '66%' : '100%' }}
                  />
                </div>
                ...
                {[
                  ...
                  return (
                    <div key={s.id} className={`flex flex-col lg:flex-row items-center gap-2 lg:gap-4 transition-all duration-500 ${isActive ? 'scale-105' : 'opacity-40'}`}>
                      <div className={`w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl flex items-center justify-center text-sm lg:text-base transition-all duration-500 border shadow-sm flex-shrink-0
            ...
            <section className="animate-fade-in w-full max-w-lg">
              ...
              <div className="glass-card p-6 lg:p-10">
                <div className="space-y-6">
                  ...
                  <button 
                    onClick={() => phoneNumber && setStep('amount')}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 lg:gap-4">
                {[5, 10, 20, 50].map((val, i) => (
                  <button 
                    ...
                    className="glass-card p-4 lg:p-6 border border-transparent hover:border-primary/50 group animate-stagger-1 text-center"
                    style={{ animationDelay: `${i * 0.05}s` }}
                  >
                    <span className="block text-2xl lg:text-3xl font-black text-primary mb-1">{val}</span>
